...
class WSContainer extends Component {
    ...
    shouldComponentUpdate(nextProps, nextState) {
        ...
        if (localStorage.userId != '' && (context.userId == undefined || context.userId == '')) {
            ...
            if ((nextProps.context.system != undefined) && JSON.stringify(nextProps.context.system.dialog_turn_counter) != JSON.stringify(this.props.context.system == undefined
                ? 0
                : this.props.context.system.dialog_turn_counter) && JSON.stringify(nextProps.rsvrTimeInfo) == JSON.stringify(this.props.rsvrTimeInfo)) {
                this.props = nextProps;
                ConfigActions.setShowflag(false);

                MrInfoActions.controlShowFlag({roomInfoShowFlag: false, rsvrInfoShowFlag: false, rsvrCnfmShowFlag: false});
                ...
                if (this.props.node == '회의실 목록 확인') {
                    ...
                    MrInfoActions.controlShowFlag({myrsvrInfoShowFlag: true});
                } else if (this.props.node == '회의실 예약') {
                    this.confirmConferenceRoomRsvr();
                } else if (this.props.node == '회의실 예약 가능시') {
                    MrInfoActions.controlShowFlag({roomInfoShowFlag: false, rsvrInfoShowFlag: false, rsvrCnfmShowFlag: true});
                    ...
                    MrInfoActions.controlShowFlag({myrsvrInfoShowFlag: false});
                    MrInfoActions.setGridFlag({setGridFlag: false});
